...
import {
  View, Dimensions, TouchableOpacity, I18nManager,
} from 'react-native';
  ...
  componentDidMount() {
    ...
    carouselItem = (props, { item, index }) => (
      <View style={[styles.carouselItemStyle,
        { backgroundColor: index === this.state.activeSlide ? colors.primary : colors.white }]}
      >
        <TouchableOpacity
          onPress={() => {
            this._carousel.snapToItem(I18nManager.isRTL
              ? this.props.data.length - 1 - index : index);
        ...
        >
          <CustomText
            content={item.title}
            color={index === this.state.activeSlide ? colors.white : colors.blue_300}
            size={12}
